...
class CompositionDom(Composition, ABC):
    ...
    def _render_document(
        self, views: List[View], legend_entries: List[Layer]
    ...
    def _render_figure(self) -> bool:
        ...
        doc, view_elements, legend_elements = self._render_document(
            self.views, legend_entries
        )
        ...
        legend_axes = list(
            zip(legend_entries, doc.make_axes(self._figure, legend_elements))
        )
...
class CompositionGrid(CompositionDom):
    ...
    def _make_doc_legend(self, legend_entries: List[Layer]):
        ...
        legend_containers = [
            mdom.MplMargin(top=0.3, bottom=0.25, left=0.1, right=0.1, fixed=True)
        ...
        return (
            mdom.MplDocument(
                root=mdom.MplMargin(
                    mdom.MplDivider(
                        mdom.MplMargin(
                            mdom.MplGrid(legend_containers, nbreak=999),
                            left=legend_margin_h,
                            right=legend_margin_h,
                        ),
                        mdom.MplGrid(view_containers, nbreak=self.nbreak),
                        vertical=True,
                        division=0.65,
                        fixed=True,
                    ),
